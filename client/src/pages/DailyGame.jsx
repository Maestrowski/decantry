...
const DailyGame = () => {
    ...
    const [score, setScore] = useState(100); // Always 100 for daily if correct
    ...
    useEffect(() => {
        ...
        // Check if already played today (local storage)
        const playedDate = localStorage.getItem('daily_played_date');
        if (playedDate === today) {
            setGameStatus('finished');
            setFeedback('You have already played today.');
            return;
        }

        const fetchGameData = async () => {
            try {
                const response = await fetch(`${API_URL}/api/game/daily`);
                if (!response.ok) {
    ...
    const handleGuess = async () => {
        ...
        if (guess.toLowerCase().trim() === correctAnswer.toLowerCase()) {
            ...
            localStorage.setItem('daily_played_date', new Date().toISOString().slice(0, 10));
            ...
            if (token) {
                try {
                    await fetch(`${API_URL}/api/user/score`, {
                        ...
                        body: JSON.stringify({ mode: 'daily', points: score })
                    });
                } catch (err) {
    ...
    if (gameStatus === 'finished') {
        return (
            <div style={{ textAlign: 'center', marginTop: '50px' }}>
                ...
                <p>{feedback || "Come back tomorrow!"}</p>
                <button
                    onClick={() => navigate('/')}
                    style={{
                        marginTop: '20px',
    ...
    return (
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', width: '100%', maxWidth: '600px', margin: '0 auto', padding: '20px' }}>
            <h2 style={{ fontSize: '2rem', marginBottom: '20px' }}>Daily Challenge</h2>
